...
class MetricsReport:
    ...
    def get_train_score(self) -> float:
        return self._res["train"]["r2"] if "r2" in self._res["train"] else self._res["train"]["f1"]
...
class SeedsMetricsReport:
    ...
    def print_result(self) -> dict:
        res = {split: {k: float(np.around(self._agg_res[split][k], 4)) for k in self._agg_res[split]} for split in
               ["val", "test"]}
        print("=" * 100)
        print("EVAL RESULTS:")
        print("[val]")
...
def _get_labels_and_probs(
    ...
    if prediction_type == PredictionType.LOGITS:
        ...
        raise Exception(f'Unexpected prediction type, got {prediction_type}')
...
def calculate_metrics(
    ...
    if task_type == TaskType.REGRESSION:
        ...
        print('log: predicted labels: ', np.unique(labels))
        result = cast(
